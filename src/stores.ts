...
export const zoom_factor = derived(
  zoom_level,
  $zoom_level => SCALE_DEFAULT + $zoom_level * SCALE_STEP,
  0
);
export const zoom_percent = derived(
  zoom_factor,
  $zoom_factor => `${Math.round(($zoom_factor / SCALE_DEFAULT) * 100)}%`,
  '100%'
);
