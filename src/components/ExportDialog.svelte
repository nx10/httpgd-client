<script context="module" lang="ts">
  ...
  export function close_export_dialog(): void {
    getModal(MODAL_ID).close();
  }
  export function open_export_dialog(): void {
...
<Modal id={MODAL_ID}>
  ...
  <div class="export-options">
    <div class="export-tools" style="float: left;">
      Size:
      <NumInput value="800" min={MIN_PREVIEW_SIZE} max={MAX_PREVIEW_SIZE} />
      <span>&times;</span>
      <NumInput value="600" min={MIN_PREVIEW_SIZE} max={MAX_PREVIEW_SIZE} />
      <span>~</span>
      <NumInput value="100" min={MIN_PREVIEW_ZOOM} max={MAX_PREVIEW_ZOOM} />
      <span style="font-size: 20;">%</span>
    </div>

    <div class="export-tools" style="float: right;">
      <Select bind:selected_id={selected_renderer_id} items={renderers_items} />
      <Button value="Open" />
