<script lang="ts">
  ...
  function plots_navigate(offset: number) {
    ...
    $httpgd_selected_plot =
      $httpgd_plots.plots[index % $httpgd_plots.plots.length].id;
  }

  function plots_navigate_newest() {
    if (!$httpgd_plots?.plots?.length) return;
    $httpgd_selected_plot =
      $httpgd_plots.plots[$httpgd_plots.plots.length - 1].id;
  }
  ...
  $: png_renderer_available =
    $httpgd_renderers?.findIndex(v => v.id === 'png') !== -1;
  ...
  <div id="tb-tools">
    <span>
      <ToolbarItem
        title="Previous"
        shortcut="&larr;"
        on:click={() => plots_navigate(-1)}>{@html iconArrowLeft}</ToolbarItem
      >
      <ToolbarItem title="Newest" shortcut="N" on:click={plots_navigate_newest}
        >{plot_index_nav_string}</ToolbarItem
      >
      <ToolbarItem
        title="Next"
        shortcut="&rarr;"
        on:click={() => plots_navigate(1)}>{@html iconArrowRight}</ToolbarItem
      >
    </span>
    <span>
      ...
      >
      <ToolbarItem title="Reset zoom" shortcut="0" on:click={() => zoomReset()}
        >{$zoom_percent}</ToolbarItem
      >
      ...
      <ul>
        ...
        <li>
          <ToolbarListItem
            title="Copy PNG"
            shortcut="C"
            on:click={close_drop_and(copyPNG)}>{@html iconCopy}</ToolbarListItem
          >
        </li>
        <li>
          <ToolbarListItem
